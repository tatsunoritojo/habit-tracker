...
## 5. 追加実装: 習慣記録UXの改善 (2025-12-07)
ユーザーからのフィードバックに基づき、習慣記録時のインタラクションを改善しました。

### 5-1. 変更内容
1.  **「統計のみ確認」オプションの追加**:
    -   未記録のカードをタップした際のダイアログに、「記録する」「キャンセル」に加えて「**統計のみ確認**」ボタンを追加しました。
    -   これにより、記録をつけずに詳細データを確認したい場合の手順が簡略化されました。
2.  **長押しによるショートカット**:
    -   カードを**長押し**することで、ダイアログを経由せずに直接詳細画面（統計ページ）へ遷移できるようになりました。

### 5-2. 目的
-   「記録はまだしないが、現状のストリークや過去のデータを確認したい」というユースケースにおける操作性を向上させるため。
