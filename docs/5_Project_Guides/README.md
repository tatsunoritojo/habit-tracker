...
**最終更新日**: 2025-12-08
**最終更新者**: Claude Code (Phase 9.5完了時点)
...
**カード管理強化（Phase 9.5）**
- **ニックネーム機能**: ユーザー表示名の設定、エール送信者として表示
- **カード重複防止**: 完全一致・類似度検出で重複カード作成を防止
- **カード作成上限**: アクティブカード最大50枚
- **公開設定細分化**: 「エールを受け取る」「テンプレートとして公開」を独立したオプションに
- **タイトル正規化**: 前後空白の自動削除
...
│   │   ├── useUserProfile.ts     # ユーザープロファイル管理（Phase 9.5）
│   │   ├── useUserDisplayName.ts # 他ユーザー表示名取得（Phase 9.5）
│   │   └── usePublicCards.ts     # 公開カード取得（Phase 9.5）
│   ├── lib/
...
│   │   ├── cardDuplicateChecker.ts # カード重複検出（Phase 9.5）
│   │   └── habitSimilarity.ts    # 類似度計算
│   └── types/
